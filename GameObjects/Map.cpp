...
void Map::loadMap(std::string path){
	...
	for (int i = 0; i < width; i++){
		for (int j = 0; j < height; j++){
            ...
            cur += ".png";
            if (typeGround < -9 || typeGround > 19) std::cout << "Map chua thong so khong hop le" << std::endl;
            grounds[i][j].setGround(cur, j * GROUND_WIDTH, i * GROUND_WIDTH, typeGround);
        }
	}
    map >> isBackground2;
    int totalCoins;
    // đọc số coin trong map
    map >> totalCoins;
    ...
    int x, y, _type;
    map >> x >> y >> _type;
    player = new Player({x, y}, _type); //khởi tạo người chơi
    ...
    for (int i = 0; i < tolCur; i++){
        ...
        enemies.push_back(new Enemies(_type, _pos, _start, _end)); //_type, _pos, _start, _end
    }
...
void Map::Buttom_and_Stone(std::list<Buttom*>& buttoms, std::list<Stone*>& stones, const Uint32& deltaTime) {
	for (auto& buttom : buttoms) {
		for (auto& stone : stones) {
...
void Map::Buttom_and_Player(std::list<Buttom*>& buttoms, Player* player, const Uint32& deltaTime) {
	if (player == nullptr) {
		return;
	}
	for (auto& buttom : buttoms) {
