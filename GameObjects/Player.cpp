...
void Player::Update(const Uint32& deltaTime){
    ...
    if (!moveQuick){
        ...
        if (currentKeyStates[SDL_SCANCODE_UP] && canJump) {
    ...
    if (canJump == false && moveQuick == false){
        velocity.second += 981.0f * (float) deltaTime / 1000;
    }
    if (timeClickS >= 0){
...
bool Player::updatePushStone(const Uint32& deltaTime) {
...
bool Player::updateMoveQuick(const Uint32& deltaTime) {
	if (moveQuick == false) {
		return false;
	}
